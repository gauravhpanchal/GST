...
@Component({
  selector: 'app-tax-invoice-for-other',
  templateUrl: './tax-invoice-for-other.component.html',
  styleUrls: ['./tax-invoice-for-other.component.scss']
})
export class TaxInvoiceForOtherComponent implements OnInit, OnChanges {
  ...
  setData(obj: any) {
    if (obj) {
      if (obj.Invoice) {
        ...
        if (obj.Invoice?.Buyer_Details && obj.Invoice.Buyer_Details.Affix_Authorised_Signatory == 'Yes') {
      ...
      if (obj["Organisation Info"]) {
        this.hsn_code = obj["Organisation Info"].Product_HSN_number_Items.join(', ')
      }
      ...
      if (obj.LeadsForStateSection) {
        ...
        if (isSellerDetailsSelected) {
          ...
          if (obj.LeadsForStateSection.NameAsSite == 'KA2') { NameAsSite = 'KA1' }
          else if (obj.LeadsForStateSection.NameAsSite == 'MH2') { NameAsSite = 'MH1' }
          const SellerDetail = this.service.GetSiteAddressBySiteID(NameAsSite);

          if (SellerDetail) {
            let address1: string;
            if (SellerDetail.DoorNoPremisesNo) { address1 = SellerDetail.DoorNoPremisesNo }
            if (SellerDetail.FloorNo) { address1 = address1 ? address1 + ", " + SellerDetail.FloorNo : SellerDetail.FloorNo }

            let address2: string;
            if (SellerDetail.BuildingPremiseName) { address2 = SellerDetail.BuildingPremiseName }
            if (SellerDetail.AreaStreetRoadName) { address2 = address2 ? address2 + ", " + SellerDetail.AreaStreetRoadName : SellerDetail.AreaStreetRoadName }
            this.seller_details = {
              business_name: SellerDetail.BUSINESS_NAME,
              address1: address1,
              address2: address2,
              address3: SellerDetail.SubLocalityLocality,
              address4: SellerDetail.City + ", " + SellerDetail.States + " - " + SellerDetail.Pincode,
  ...
  updateGST(value: string, key: string) {
    if (!Number.isNaN(parseInt(value)) || value == "") {
      ...
      let gst = '0';
      if (parseFloat(value)) {
        amount = (parseFloat(value) * this.total_bill_before_gst / 100).toString();
        gst = parseFloat(value).toString();
      }

      if (key === 'cgst') { this.cgst_amount = amount, this.cgst = gst }
      else if (key === 'sgst') { this.sgst_amount = amount, this.sgst = gst }
      else if (key === 'igst') { this.igst_amount = amount, this.igst = gst }

      this.gst_amount = (parseFloat(this.cgst_amount) + parseFloat(this.sgst_amount) + parseFloat(this.igst_amount)).toString()
      this.gst_amount_in_words = amountToWords(this.gst_amount)
      this.total_bill = this.total_bill_before_gst + parseFloat(this.gst_amount)
